'''
Hello again everyone! This is Sreekanth Kavali
...
# ... (Feature extraction and data preprocessing steps remain the same)
...
# Encode the "merchant" and "category" columns using LabelEncoder
# ... (Label encoding and one-hot encoding steps remain the same)

# ... (Standardization and combining encoded features remain the same)
...
# Check for and handle missing values in the target variable (y_train)
if y_train.isnull().sum() > 0:
    y_train = y_train.fillna(0)  # Fill NaN values with 0 or impute accordingly
...
# Apply Incremental PCA for dimensionality reduction
# ... (Incremental PCA fitting and transformation remain the same)
...
# Calculate model metrics for evaluation
accuracy = accuracy_score(y_test, y_pred)
confusion = confusion_matrix(y_test, y_pred)
report = classification_report(y_test, y_pred)
